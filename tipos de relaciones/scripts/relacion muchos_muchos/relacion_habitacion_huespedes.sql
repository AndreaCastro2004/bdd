...
--agregacion 1
select ha.habitacion_numero, count (re.huesped_id) as cantidad_huespedes
...
--agregacion 
select ha.habitacion_numero,round(AVG(re.huesped_id),2)  as promedio_huespedes from habitaciones ha,reservas re
where ha.habitacion_numero=re.habitacion
group by ha.habitacion_numero
--CONSULTAS 3
select ha.habitacion_numero,hu.nombres,hu.apellidos
...
--agregacion 
select habitacion_numero,sum(precio_por_noche) as suma_precio_por_noche 
from habitaciones ha,reservas re
where ha.habitacion_numero=re.habitacion
